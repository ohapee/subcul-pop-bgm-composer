/**
 * サブカルポップBGMコンポーザー - ストレージ管理
 */

const LAST_STATE_KEY = 'subcul-pop-bgm-laststate-v1';
const PRESETS_KEY = 'subcul-pop-bgm-presets-v1';
...
export function serializeState(state) {
  return {
    ...
    subculStyle: state.subculStyle || 'none',
    insts: [...state.insts],
    sfx: [...state.sfx],
    density: state.density || 'occasional',
    focusMode: state.focusMode || 'flow',
    negatives: [...state.negatives],
    key: state.key || 'marusa_eb',
    tempo: Number(state.tempo) || 96,
...
export function exportPresetsAsJSON() {
  ...
  a.setAttribute('download', `subcul_pop_presets_${dateStr}.json`);
